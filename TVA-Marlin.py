...
Aplicaciçon: TVA-Marlin Test de Velocidad y Aceleración para firmware Marlin v1.0.0
...
class PrinterConnection:
    ...
    def open(self):
        try:
            self.ser = serial.Serial(self.port, self.br, timeout=self.timeout)
        except IOError:
            self.ser.close()
        time.sleep(1)
...
class GUI:
    ...
    def populate_root(self):
        ...
        Button(self.frameI, command=self.start, text='START!', bg='green', fg='white').pack()
        Label(self.frameI, text='  ').pack()
